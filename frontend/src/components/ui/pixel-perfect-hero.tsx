...
export function PixelHero({
  ...
  useEffect(() => {
    ...
    const muted = getComputedStyle(div).color || "#94a3b8";
    div.className = "text-primary";
    const primary = getComputedStyle(div).color || "#3b82f6";
  ...
  return (
    <div className="relative w-full h-[60dvh] bg-background flex flex-col justify-center items-center px-4 overflow-hidden select-none isolate text-white">
      <style>{`
        @keyframes marquee {
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,#0f1117_100%)] pointer-events-none opacity-80" />
      </div>

      {/* Top Container: Tahoe Glass Header */}
      <div className="flex flex-col items-center justify-center text-center z-10 w-full mb-6">
        <h1 className="tahoe-glass-text flex flex-row items-center justify-center gap-1.5 sm:gap-4 lg:gap-6 px-1 w-full flex-wrap text-5xl sm:text-6xl md:text-8xl lg:text-9xl leading-none">
      ...
      <div className="flex flex-col items-center justify-center text-center z-10 w-full mb-8">
        <p className="text-sm sm:text-lg md:text-xl font-light text-gray-300 max-w-[95%] sm:max-w-md md:max-w-xl px-1 leading-relaxed">
          {description}
        </p>
      </div>
      ...
      <div
        className={cn("pointer-events-auto flex flex-row items-center justify-center gap-3 z-10 transition-all duration-1000 transform", isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")}
        style={{ transitionDelay: "450ms" }}
      >
        <button onClick={onPrimaryClick} className="relative inline-flex h-10 md:h-12 items-center justify-center gap-1.5 md:gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 px-4 md:px-8 text-xs md:text-sm font-semibold text-white shadow-lg transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer">
          <span className="inline">{primaryCta}</span>
          <ArrowRight className="w-4 h-4" />
        </button>
        <a href={githubUrl} target="_blank" rel="noopener noreferrer" onClick={onSecondaryClick} className="relative inline-flex h-10 md:h-12 items-center justify-center gap-1.5 md:gap-2 rounded-xl bg-gray-800 hover:bg-gray-700 px-4 md:px-8 text-xs md:text-sm font-semibold text-white shadow-lg border border-gray-600 transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer">
          <GitBranch className="w-4 h-4" />
          <span className="inline">{secondaryCta}</span>
        </a>
      </div>
    </div>
  );
